...
class ConversationVideoUploadController extends Controller
{
    ...
    {
        $key = 'tmp/media/'.$conversation->workspace_id.'/'.Str::uuid().'.mp4';
    ...
    public function store(StoreConversationVideoRequest $request, Conversation $conversation): JsonResponse
    {
        ...
        // tmp prefix entirely.
        $this->authorizeKey($key, (string) $conversation->workspace_id);
        ...
        if ($size > Platform::maxVideoBytesCeiling()) {
            ...
            Log::warning('Rejected oversize video upload', ['conversation_id' => $conversation->id, 'size' => $size]);
        ...
        if (! $this->looksLikeMp4($disk, $key)) {
            Storage::disk($disk)->delete($key);
            Log::warning('Rejected non-MP4 video upload', ['conversation_id' => $conversation->id]);
            abort(422, 'Uploaded file is not a valid MP4 video.');
        }

        $final = 'media/'.$conversation->workspace_id.'/'.Str::uuid().'.mp4';
